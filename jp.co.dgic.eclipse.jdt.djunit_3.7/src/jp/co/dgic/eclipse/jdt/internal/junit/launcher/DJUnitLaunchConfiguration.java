...
//	}
	...
	private void addRequiredJars(ILaunchConfiguration configuration, List<String> junitEntries, URL djunitURL)
			throws IOException, CoreException {

		IJavaProject javaProject = getJavaProject(configuration);
		if ((javaProject == null) || !javaProject.exists()) {
			abort(
					JUnitMessages.JUnitLaunchConfigurationTab_error_projectnotexists,
					null,
...
//				new URL(djunitURL, LIB_DIR + "bcel-r643711.jar")).getFile());
	...
	public String getVMArguments(ILaunchConfiguration configuration)
		...
		// path to djunit-premain.jar
		URL url = Platform.getBundle(DJUnitPlugin.PLUGIN_ID).getEntry("/");
		String premainJarPath = null;
		try {
			premainJarPath = new File(FileLocator.toFileURL(new URL(url, LIB_DIR + "djunit-premain.jar")).getFile()).getAbsolutePath();
	...
	private String getSourceDirectories(IJavaProject javaProject)
		...
		Set<IPath> dirs = new HashSet<IPath>();
		Set<String> checkedProjects = new HashSet<String>(); // for checking cycle reference
		findSourceDirs(javaProject, dirs, checkedProjects);

		StringBuffer sb = new StringBuffer();

		for (Iterator<IPath> it = dirs.iterator(); it.hasNext();) {
			sb.append(it.next());
			sb.append(";");
		}
	...
	private void findSourceDirs(IJavaProject javaProject, Set<IPath> sourceDirs,
			Set<String> checkedProjects) throws CoreException {
	...
	private void findSourceDirFrom(IJavaProject javaProject, Set<IPath> sourceDirs)
			throws CoreException {
