...
public abstract class AbstractAsmMethodVisitor15x extends CodeAdapter implements Constants {
	...
	protected CodeVisitor mv;

	public AbstractAsmMethodVisitor15x(CodeVisitor cv, String className, String methodName, String desc, boolean isStatic, String[] exceptions, int maxLocals, String[] superClassNames) {
		super(cv);
	...
	public void createMethodCall(int opcode, String owner, String name, String desc) {
		...
		boolean isStaticMethod = (opcode == INVOKESTATIC);
...
//    		mv.visitLabel(toReturn);
	...
	protected String[] getExceptions(String className, String methodName, String desc) {
		...
		AsmClassChecker15x acc = getClassChecker(className);
	...
	protected AsmClassChecker15x getClassChecker(String className) {

		AsmClassChecker15x acc = (AsmClassChecker15x) classCheckerMap.get(className);

		if (acc != null) return acc;

//		acc = new AsmClassChecker15x();
		try {
			AsmClassReader15x cr = new AsmClassReader15x(className);
			acc = AsmClassChecker15x.getInstance(className, cr);
//			cr.accept(acc);
		} catch (IOException e) {
			// continue
		} catch (Throwable t) {
...
//			t.printStackTrace();
	...
	protected int getLoadOpcodeByType(Type type) {
		if (type.equals(Type.BOOLEAN_TYPE)) return ILOAD;
		if (type.equals(Type.BYTE_TYPE)) return ILOAD;
		if (type.equals(Type.CHAR_TYPE)) return ILOAD;
		if (type.equals(Type.SHORT_TYPE)) return ILOAD;
		if (type.equals(Type.INT_TYPE)) return ILOAD;
		if (type.equals(Type.LONG_TYPE)) return LLOAD;
		if (type.equals(Type.DOUBLE_TYPE)) return DLOAD;
		if (type.equals(Type.FLOAT_TYPE)) return FLOAD;
		return ALOAD;
	}

	protected int getStoreOpcodeByType(Type type) {
		if (type.equals(Type.BOOLEAN_TYPE)) return ISTORE;
		if (type.equals(Type.BYTE_TYPE)) return ISTORE;
		if (type.equals(Type.CHAR_TYPE)) return ISTORE;
		if (type.equals(Type.SHORT_TYPE)) return ISTORE;
		if (type.equals(Type.INT_TYPE)) return ISTORE;
		if (type.equals(Type.LONG_TYPE)) return LSTORE;
		if (type.equals(Type.DOUBLE_TYPE)) return DSTORE;
		if (type.equals(Type.FLOAT_TYPE)) return FSTORE;
		return ASTORE;
	}

	protected int getReturnOpcodeByType(Type type) {
		if (type.equals(Type.BOOLEAN_TYPE)) return IRETURN;
		if (type.equals(Type.BYTE_TYPE)) return IRETURN;
		if (type.equals(Type.CHAR_TYPE)) return IRETURN;
		if (type.equals(Type.SHORT_TYPE)) return IRETURN;
		if (type.equals(Type.INT_TYPE)) return IRETURN;
		if (type.equals(Type.LONG_TYPE)) return LRETURN;
		if (type.equals(Type.DOUBLE_TYPE)) return DRETURN;
		if (type.equals(Type.FLOAT_TYPE)) return FRETURN;
		return ARETURN;
	}
